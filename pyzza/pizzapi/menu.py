...
class Menu(object):
    ...
    def __init__(self, data={}, country=COUNTRY_USA):
        ...
        if self.variants:
            ...
            self.preconfigured = self.parse_items(
    ...
    def from_store(cls, store_id, lang='en', country=COUNTRY_USA):
        response = request_json(Urls(country).menu_url(),
                                store_id=store_id, lang=lang)
        menu = cls(response)
        return menu
    ...
    def build_categories(self, category_data, parent=None):
        ...
        for product_code in category_data['Products']:
            if product_code not in self.menu_by_code:
                raise Exception('PRODUCT NOT FOUND: %s %s' %
    ...
    def search(self, **conditions):
        def max_len(x): return 2 + max(len(v[x])
                                       for v in list(self.variants.values()))
        for v in self.variants.values():
            v['Toppings'] = dict(x.split('=', 1)
                                 for x in v['Tags']['DefaultToppings'].split(',') if x)
            if all(y in v.get(x, '') for x, y in conditions.items()):
                print(v['Code'], end=' ')
                print(v['Name'].encode(
                    'ascii', 'ignore').decode('ascii'), end=' ')
                print('$' + v['Price'], end=' ')
